...
# --- Vectorize data ---
  ...
  tmp <- data.frame(ID = ID, IND_ID = IND_ID, Y = as.vector(Y), Yrecap = as.vector(Yrecap),
                    DL = as.vector(DL), UL = as.vector(UL),
  ...
  output <- tmp[is.na(tmp$Yrecap) == 0,] # remove individuals that were not captured in the previous trial
