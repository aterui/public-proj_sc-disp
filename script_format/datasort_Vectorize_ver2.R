...
# Data ----
## Define data source
d1 <- read.csv("data_raw/Indian_final.csv"); d1 <- d1[,1:which(colnames(d1)=="COMMENTS")]; d1$Stream <- "Indian"
d2 <- read.csv("data_raw/Todd_final.csv"); d2 <- d2[,1:which(colnames(d2)=="COMMENTS")]; d2$Stream <- "Todd"
dat <- rbind(d1, d2)
dat <- dat[dat$VALID==Y,]
